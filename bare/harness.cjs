...
globalThis.bench = function bench (name, fn) {
  ...
  while (elapsed < maximumSamplingTime) {
    ...
    /*
     * This is problematic because the various benchmarks
     * might take too few samples before the optimizations kicks in.
     * Specifically, the "classes" example timed out instead of hitting
     * confidence. This block has therefore been commented out.
    if (samples.length > 2) {
